...
def triangulate_all(config_dict):
    ...
    # print("Multi person: ", multi_person)
    # print("Pose model: ", pose_model)
    # print("Frame range: ", frame_range)
    # print("likelihood_threshold: ", likelihood_threshold)
    # print("interpolation kind: ", interpolation_kind)
    # print("Interp gap smaller than: ", interp_gap_smaller_than)
    # print("Fill large gaps with: ", fill_large_gaps_with)
    # print("Show interp indices: ", show_interp_indices)
    # print("Undistort points: ", undistort_points)
    # print("Make c3d: ", make_c3d)
    ...
    # print("Calibration params: ", calib_params)
    # print("Rotation original:" , calib_params['R'])
    # print("Translation original :" , calib_params['T']) 

    # # Original rotation and translation
    # R1_euler, R2_euler = calib_params['R']  # These are actually Euler angles, not matrices
    # T1, T2 = calib_params['T']  # These are already correct

    # # Convert Euler angles to 3×3 rotation matrices
    # R1, _ = cv2.Rodrigues(R1_euler)  # Convert to rotation matrix
    # R2, _ = cv2.Rodrigues(R2_euler)  # Convert to rotation matrix

    # # Compute new rotation and translation for Cam2
    # R_new2 = R1.T @ R2  # Equivalent to R1^(-1) * R2
    # T_new2 = R1.T @ (T2 - T1)  # Equivalent to R1^(-1) * (T2 - T1)

    # # Ensure correct formats
    # R_new2 = np.array(R_new2, dtype=np.float64)  # Force 3×3 matrix
    # T_new2 = np.array(T_new2, dtype=np.float64).reshape(3)  # Ensure it's a 3D vector

    # # Cam1 is now at the origin
    # R_new1 = np.eye(3, dtype=np.float64)  # Identity matrix for rotation
    # T_new1 = np.zeros(3, dtype=np.float64)  # Zero translation

    # # Update calib_params
    # calib_params['R'] = [R_new1, R_new2]  # Store as 3×3 matrices
    # calib_params['T'] = [T_new1, T_new2]  # Store as 3D vectors

    # print("Rotation new:" , calib_params['R'])
    # print("Translation new:" , calib_params['T']) 
    ...
    print("Nb persons to detect: ", nb_persons_to_detect)
    ...
    for f in tqdm(range(*f_range)):
        ...
        # print("likelihood_files: ", likelihood_files)
        ...
        for n in range(nb_persons_to_detect):
            ...
            # Triangulate cameras with min reprojection error
                ...
                # print("coords_2D_kpt: ", coords_2D_kpt)
                ...
                # print('coords_2D_kpt ', coords_2D_kpt)
                # print('coords_2D_kpt_swapped ', coords_2D_kpt_swapped)
    ...
    print("Deleted person id: ", deleted_person_id)
